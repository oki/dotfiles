...
exec "Snippet aa <% ".st.et." %>".st.et
exec "Snippet a= <%= ".st.et." %>".st.et
exec "Snippet a- <%- ".st.et." %>".st.et
exec "Snippet asl <%= javascript_include_tag :defaults %><CR><%= active_scaffold_includes %><CR><CR><%= yield %>"
exec "Snippet as active_scaffold"
exec "Snippet asc active_scaffold :".st.et." do |config|<CR>".st.et."<CR>end"
exec "Snippet asb active_scaffold :book do |config|<CR>\tconfig.list.columns = [:author, :title, :isbn, :descr, :user]<CR>\tconfig.create.columns.exclude :user<CR>\tconfig.action_links.add 'index', :controller => 'users', :label => 'Users', :page => true<CR>\tconfig.action_links.add 'free', :label => 'Free', :type => :record, :position => false<CR>end"
exec "Snippet asu active_scaffold :user do |config|<CR>\tconfig.list.columns = [:first, :last]<CR>\tconfig.update.columns.exclude :books<CR>\tconfig.create.columns.exclude :books<CR>\tconfig.action_links.add 'index', :controller => 'books', :label => 'Books', :page => true<CR>end"
exec "Snippet free def free<CR>\t@record = Book.find params[:id]<CR>\t@record.user = nil<CR>\t@record.save<CR><CR>\trender :update do |page|<CR>\t\tpage.replace element_row_id(:action => 'list'), :partial => 'list_record', :object => @record<CR>\tend<CR>end"
exec "Snippet vpo validates_presence_of"
